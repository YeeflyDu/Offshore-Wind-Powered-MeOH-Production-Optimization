...
CAPEX_BAT = [153, 110, 89, 76, 68, 61]; % Battery capacity cost, ву/kWh
CAPEX_BAT = CAPEX_BAT(1,Currunt_Year);
OPEX_BAT = [2.6, 2.2, 2.05, 1.9, 1.77, 1.71] + 0.0002; % Battery O&M cost, ву/kWh
OPEX_BAT = OPEX_BAT(1,Currunt_Year);
CAPEX_BAT_INT = [76, 55, 44, 37, 33, 30]; % Battery interface cost, ву/kW
CAPEX_BAT_INT = CAPEX_BAT_INT(1,Currunt_Year);
OPEX_BAT_INT = [1.29, 1.10, 1.01, 0.93, 0.86, 0.84]; % Battery interface O&M cost, ву/kW
OPEX_BAT_INT = OPEX_BAT_INT(1,Currunt_Year);
BAT_EFF = [0.92, 0.93, 0.94, 0.95, 0.95, 0.95].^0.5; % Battery charge/discharge efficiency
BAT_EFF = BAT_EFF(1,Currunt_Year);
% Variables
...
P2A_DAC = WACC*(1+WACC)^Lifetime_DAC(1,Currunt_Year)/((1+WACC)^Lifetime_DAC(1,Currunt_Year)-1); % DAC annualization factor
CAPEX_DAC = [20833, 7096, 4914, 3653, 3084, 2759]*1000*730/2378; % DAC capacity cost, ву/tCO2h
CAPEX_DAC = CAPEX_DAC(1,Currunt_Year);
OPEX_DAC = [31, 18.5, 13.7, 10.7, 9.3, 8.5] + 1; % DAC variable O&M cost, ву/tCO2
OPEX_DAC = OPEX_DAC(1,Currunt_Year);
P2CO2 = [700, 309, 275, 250, 237, 229]*250/700; % DAC electricity demand, kWh/tCO2
P2CO2 = P2CO2(1,Currunt_Year);
T2CO2 = [3000, 1326, 1179, 250, 237, 229]*1750/3000; % DAC thermal demand, kWh/tCO2
T2CO2 = T2CO2(1,Currunt_Year);
% Variables
...
CAPEX_EWB = [65, 60, 60, 60, 60, 60]; % EWB capacity cost, ву/kW
CAPEX_EWB = CAPEX_EWB(1,Currunt_Year);
OPEX_EWB = [0.0005, 0.0005, 0.0005, 0.0004, 0.0004, 0.0004]; % EWB variable O&M cost, ву/kWh
OPEX_EWB = OPEX_EWB(1,Currunt_Year);
% Variables
...
CAPEX_ES = [446, 316, 234, 189, 163, 148]; % Electrolyzer capacity cost, ву/kW
CAPEX_ES = CAPEX_ES(1,Currunt_Year);
OPEX_ES = [0.0014, 0.0011, 0.0009, 0.0009, 0.0009, 0.0009] + 0.0002; % Electrolyzer variable O&M cost, ву/kWhH2
OPEX_ES = OPEX_ES(1,Currunt_Year);
P2H_EFF = [0.748, 0.762, 0.777, 0.791, 0.806, 0.821];
P2H_EFF = P2H_EFF(1,Currunt_Year);
P2T_EFF = [0.214, 0.2, 0.187, 0.173, 0.16, 0.147];
P2T_EFF = P2T_EFF(1,Currunt_Year);
% Variables
...
CAPEX_HC =[2500, 1900, 1900, 1900, 1900, 1900]; % ву/kW
CAPEX_HC = CAPEX_HC(1,Currunt_Year);
...
CAPEX_HS = [493, 483, 483, 483, 483, 483]*1000; % H2 storage capacity cost, ву/tH2
CAPEX_HS = CAPEX_HS(1,Currunt_Year);
...
%% OWF
C_WT = [50.49, 45.44, 40.39, 38.21, 36.04, 33.86]/1000; % ву/kWh
C_WT = C_WT(1,Currunt_Year);
...
%% Constant
Constraints = [];
Constraints = [Constraints; P_WT'+P_BAT_DIS==P_BAT_CHA+P_DAC+P_CC+P_ES+P_HC+P_MeOH+P_EWB]; % Power balance
% Battery
...
Constraints = [Constraints; TEMP_BAT_CHA+TEMP_BAT_DIS==1]; % Single state constraint (charge or discharge only)
for month = 1 : Typical_Day
    for hour = 1 : 24
        Constraints = [Constraints; 0.05*Q_BAT<=0.5*Q_BAT+(sum(-P_BAT_DIS(month,1:hour)/BAT_EFF+P_BAT_CHA(month,1:hour)*BAT_EFF))<=0.95*Q_BAT]; % SOC constraints
    end
    Constraints = [Constraints; -sum(P_BAT_DIS(month,:),2)/BAT_EFF+sum(P_BAT_CHA(month,:),2)*BAT_EFF==0]; % Daily SOC balance
end
% DAC
Constraints = [Constraints; Q_DAC>=0; P_DAC>=0];
Constraints = [Constraints; 0<=CO2_DAC<=Q_DAC; P_DAC==CO2_DAC*P2CO2; CO2_DAC-CO2_CC+CO2_CS_OUT>=1.46*MeOH]; % Capacity constraints, power consumption, CO2 balance
% Electric Water Boiler (EWB)
...
for month = 1 : Typical_Day
    for hour = 1 : 24
        Constraints = [Constraints; 0.05*Q_TS<=0.5*Q_TS+(sum(-T_TS_OUT(month,1:hour)+T_TS_IN(month,1:hour)))<=0.95*Q_TS]; % SOC constraints
    end
end
Constraints = [Constraints; -sum(T_TS_OUT,2)+sum(T_TS_IN,2)==0]; % Daily SOC balance
% CO2 Compressor (CC)
Constraints = [Constraints; Q_CC>=0; P_CC>=0];
Constraints = [Constraints; 0<=CO2_CC<=Q_CC; P_CC==99*CO2_CC]; % Capacity constraints, power consumption
% CO2 Storage (CS)
Constraints = [Constraints; Q_CS>=0; CO2_CS_OUT>=0; CO2_CC>=0];
Constraints = [Constraints; CO2_CC<=1/6*Q_CS]; % CO2 balance
for month = 1 : Typical_Day
    for hour = 1 : 24
        Constraints = [Constraints; 0.05*Q_CS<=0.5*Q_CS+(sum(-CO2_CS_OUT(month,1:hour)+CO2_CC(month,1:hour)))<=0.95*Q_CS]; % SOC constraints
    end
end
Constraints = [Constraints; -sum(CO2_CS_OUT,2)+sum(CO2_CC,2)==0]; % Daily SOC balance
% Electrolysis System (ES)
Constraints = [Constraints; Q_ES>=0; H2_ES>=0];
Constraints = [Constraints; 0.25*Q_ES<=P_ES<=Q_ES; H2_ES-H2_HC+H2_HS_OUT>=0.199*MeOH]; % Capacity constraints, power consumption, H2 balance
Constraints = [Constraints; H2_ES==3.6/141860*P_ES*P2H_EFF];
for month = 1 : Typical_Day
    for hour = 2 : 24
        Constraints = [Constraints; P_ES(month,hour)-P_ES(month,hour-1)<=0.5*P_ES(month,hour-1)]; % Ramp-up constraint
        Constraints = [Constraints; P_ES(month,hour-1)-P_ES(month,hour)<=0.5*P_ES(month,hour-1)]; % Ramp-down constraint
    end
end
% H2 Compressor (HC)
Constraints = [Constraints; Q_HC>=0; H2_HC>=0];
Constraints = [Constraints; 0<=P_HC<=Q_HC; P_HC==990*H2_HC]; % Capacity constraints, power consumption
% H2 Storage (HS)
Constraints = [Constraints; Q_HS>=0; H2_HS_OUT>=0];
Constraints = [Constraints; H2_HC<=1/6*Q_HS]; % H2 balance
for month = 1 : Typical_Day
    for hour = 1 : 24
        Constraints = [Constraints; 0.05*Q_HS<=0.5*Q_HS+(sum(-H2_HS_OUT(month,1:hour)+H2_HC(month,1:hour)))<=0.95*Q_HS]; % SOC constraints
    end
end
Constraints = [Constraints; -sum(H2_HS_OUT,2)+sum(H2_HC,2)==0]; % Daily SOC balance
% MeOH Production System
Constraints = [Constraints; Q_MeOH>=0; P_MeOH>=0];
Constraints = [Constraints; 0.5*Q_MeOH<=MeOH<=Q_MeOH; P_MeOH==169*MeOH]; % Capacity constraints, power consumption
for month = 1 : Typical_Day
    for hour = 2 : 24
...
Cost_EWB = (P2A_EWB+0.015)*CAPEX_EWB*Q_EWB + OPEX_EWB*sum(P_EWB,'all')*(360/Typical_Day); % Electric Water Boiler (EWB)
Cost_TS = (P2A_TS+0.0075)*27*Q_TS + 0.0001*sum(T_TS_IN,'all')*(360/Typical_Day); % Thermal Storage (TS)
Cost_CC = (P2A_CC+0.04)*338000*Q_CC + 0.1*sum(CO2_CC,'all')*(360/Typical_Day); % CO2 Compressor (CC)
Cost_CS = (P2A_CS+0.013)*22000*Q_CS + 0.1*sum(CO2_CC,'all')*(360/Typical_Day); % CO2 Storage (CS)
Cost_ES = (P2A_ES+0.035)*CAPEX_ES*Q_ES + OPEX_ES*P2H_EFF*sum(P_ES,'all')*(360/Typical_Day); % Electrolysis System (ES)
Cost_HC = (P2A_HC+0.04)*CAPEX_HC*Q_HC + 0.0001/0.025*sum(P_HC,'all')*(360/Typical_Day); % H2 Compressor (HC)
Cost_HS = (P2A_HS+0.01)*CAPEX_HS*Q_HS + 0.0001*141860/3.6*sum(H2_HC,'all')*(360/Typical_Day); % H2 Storage (HS)
Cost_MeOH = (P2A_MeOH+0.04)*CAPEX_MeOH*Q_MeOH + 11*sum(MeOH,'all')*(360/Typical_Day); % MeOH Production System
Cost_Electricity = C_WT*sum(P_WT,'all')*(360/Typical_Day); % OWF Electricity Purchase
Cost = Cost_Battery + Cost_DAC + Cost_EWB + Cost_TS + Cost_CC + Cost_CS + Cost_ES + Cost_HC + Cost_HS + Cost_MeOH + Cost_Electricity;
%% LCOM Calculation
...
%% LCOM Result Output (Results 1,2,3 show installation, O&M, and electricity costs for different devices)
% Hydrogen production
...
Result(1,3) = value(sum((P_ES),'all'))*C_WT*(360/Typical_Day)/value(Cost)*value(LCOM); 
% Carbon capture
Result(2,1) = value((P2A_DAC)*CAPEX_DAC*Q_DAC)/value(Cost)*value(LCOM);
Result(2,2) = value((0.04)*CAPEX_DAC*Q_DAC + OPEX_DAC*sum(CO2_DAC,'all')*(360/Typical_Day))/value(Cost)*value(LCOM);
Result(2,3) = value(sum((P_DAC),'all'))*C_WT*(360/Typical_Day)/value(Cost)*value(LCOM); 
% Heat supply
...
Result(3,3) = value(sum((P_EWB),'all'))*C_WT*(360/Typical_Day)/value(Cost)*value(LCOM); 
% Methanol production
...
Result(4,3) = value(sum((P_MeOH),'all'))*C_WT*(360/Typical_Day)/value(Cost)*value(LCOM); 
% Storage
...
Result(5,3) = value(sum((P_BAT_CHA-P_BAT_DIS),'all')+sum((P_CC),'all')+sum((P_HC),'all'))*C_WT*(360/Typical_Day)/value(Cost)*value(LCOM);
% LCOM
LCOM = value(LCOM);
%% GHG Emission Intensity Output (g/MJ, monthly output)
GHG = ((value(sum(CO2_DAC,2)*30)-value(sum(MeOH,2)*30)*1.375)*1000000./(value(sum(MeOH,2)*30)*19.9*1000))';
...
CO2_DAC = value(CO2_DAC)';
CO2_DAC = reshape(CO2_DAC,1,288)';
H2_ES = value(H2_ES)';
H2_ES = reshape(H2_ES,1,288)';
MeOH = value(MeOH)';
MeOH = reshape(MeOH,1,288)';
Supply = [CO2_DAC H2_ES MeOH];