...
% Parameter
C_CO2 = [96.7, 129.1, 159.1, 189.1, 209.9, 230.6]; % External CO2 purchase price, €/tCO2 (The impact of FuelEU Maritime on European shipping)
...
Constraints = [Constraints; TEMP_BAT_CHA+TEMP_BAT_DIS==1]; % Single operation mode (charge or discharge only)
...
Constraints = [Constraints; 0<=CO2_DAC<=Q_DAC; P_DAC==CO2_DAC*P2CO2; CO2_DAC+CO2_GRID-CO2_CC+CO2_CS_OUT>=1.46*MeOH; CO2_GRID>=0]; % Capacity limits, power consumption, CO2 balance
% Electric Water Boiler (EWB)
...
Constraints = [Constraints; 0<=CO2_CC<=Q_CC; P_CC==99*CO2_CC]; % Capacity limits, power consumption
% CO2 storage (CS)
...
Constraints = [Constraints; 0.25*Q_ES<=P_ES<=Q_ES; H2_ES-H2_HC+H2_HS_OUT>=0.199*MeOH]; % Capacity limits, power consumption, H2 balance
...
Constraints = [Constraints; 0<=P_HC<=Q_HC; P_HC==990*H2_HC]; % Capacity limits, power consumption
% H2 storage (HS)
...
Constraints = [Constraints; 0.5*Q_MeOH<=MeOH<=Q_MeOH; P_MeOH==169*MeOH]; % Capacity limits, power consumption
for month = 1 : Typical_Day
    for hour = 2 : 24
...
%% Objective function
Cost_EXTRA = C_CO2*sum(CO2_GRID,'all')*(360/Typical_Day);
...
Cost_TS = (P2A_TS+0.0075)*27*Q_TS + 0.0001*sum(T_TS_IN,'all')*(360/Typical_Day); % Heat storage (TS)
...
Cost_MeOH = (P2A_MeOH+0.04)*CAPEX_MeOH*Q_MeOH + 11*sum(MeOH,'all')*(360/Typical_Day); % Methanol production system
Cost_Electricity = C_WT*sum(P_WT,'all')*(360/Typical_Day); % OWF electricity purchase
Cost = Cost_EXTRA + Cost_Battery + Cost_DAC + Cost_EWB + Cost_TS + Cost_CC + Cost_CS + Cost_ES + Cost_HC + Cost_HS + Cost_MeOH + Cost_Electricity;
%% LCOM Calculation
...
% Carbon capture
Result(2,1) = value((P2A_DAC)*CAPEX_DAC*Q_DAC)/value(Cost)*value(LCOM) + value(Cost_EXTRA)/value(Cost)*value(LCOM);
...
GHG = ((value(sum(CO2_DAC,2)*30)+value(sum(CO2_GRID,2)*30)-value(sum(MeOH,2)*30)*1.375)*1000000./(value(sum(MeOH,2)*30)*19.9*1000))';
