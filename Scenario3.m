...
for month = 1 : Typical_Day  
    Constraints = [Constraints; sum(P_GRID(month,:))*(360/Typical_Day)*CO2_EFF+sum(CO2_DAC(month,:))*(360/Typical_Day)*1000000-...
        sum(MeOH(month,:))*(360/Typical_Day)*1.375*1000000<=28.2*sum(MeOH(month,:))*(360/Typical_Day)*1000*19.9]; % 28.2g/MJ constraint
end
Constraints = [Constraints; P_WT'+P_BAT_DIS+P_GRID==P_BAT_CHA+P_DAC+P_CC+P_ES+P_HC+P_MeOH+P_EWB]; % Power balance
% Battery
...
Constraints = [Constraints; 0<=CO2_DAC<=Q_DAC; P_DAC==CO2_DAC*P2CO2; CO2_DAC-CO2_CC+CO2_CS_OUT>=1.46*MeOH]; % Capacity limits, power consumption, CO2 balance
% Electric Water Boiler (EWB)
...
for month = 1 : Typical_Day  
    GHG(1,month)=value(sum(P_GRID(month,:))*(360/Typical_Day)*CO2_EFF+sum(CO2_DAC(month,:))*(360/Typical_Day)*1000000-...
